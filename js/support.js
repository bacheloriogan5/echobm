...
function addMessage(side, message) {
  ...
  let html = "";
  if(vm.photo) {
    html = `<a class="attachment-image" style="width: 300px;" target="_blank" rel="nofollow noopener" src="/${vm.photo}">
    ...
    html = makeLinksClickable(vm.text);
  }
  ...
  axios
    ...
    .then((response) => {
      ...
      response.data.messages.forEach((v) => {
        const data = JSON.parse(v.message);
        console.log(`Text: ${v.message}`);
        if (data.text !== '!$[OPEN_SUPPORT_WINDOW]')
